...
function DirectorWords() {
  const directorData = {
    ...
    experience: "25+ Years",
    photo: "/images/director.jpg", // Replace with actual image path
    message:
      ...
      <div className="bg-white rounded-3xl shadow-2xl overflow-hidden mb-12">
        <div className="grid md:grid-cols-3 gap-0">
          <div className="md:col-span-1 bg-gradient-to-b from-[#0B3B2C] to-[#1a5a3a] p-8 flex flex-col items-center justify-center">
            <div className="w-48 h-48 rounded-full overflow-hidden border-4 border-white shadow-xl mb-4">
              <img
                ...
                onError={(e) => {
                  e.target.src =
                    "https://via.placeholder.com/200x200?text=Director";
                }}
              />
            </div>
          ...
          <div className="md:col-span-2 p-8">
            ...
            <div className="mt-8 flex items-center gap-4 text-[#0B3B2C]">
              <div className="flex items-center gap-2">
                <Mail className="w-4 h-4" />
                <span className="text-sm">{directorData.contact.email}</span>
              </div>
              <div className="flex items-center gap-2">
                <Phone className="w-4 h-4" />
                <span className="text-sm">{directorData.contact.phone}</span>
              </div>
            </div>
          </div>
        </div>
