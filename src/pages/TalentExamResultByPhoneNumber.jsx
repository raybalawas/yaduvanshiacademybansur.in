...
const TalentExamResultByPhoneNumber = () => {
  ...
  const downloadResultPDF = (singleResult) => {
    ...
    const percentage = resultToPrint.percentage || calculatePercentage(resultToPrint.marks, resultToPrint.class);
    const grade = resultToPrint.grade || getGrade(percentage);
      ...
      <body>
        ...
        <div class="result-card">
          ...
          <div class="info-row">
            ...
            <span class="value" style="color: ${resultToPrint.resultStatus === "pass" ? "green" : "red"}; font-weight: bold;">
              ${resultToPrint.resultStatus === "pass" ? "PASS" : "FAIL"}
            </span>
          </div>
        </div>
        <div class="rank">
  ...
  const renderResultCard = (resultData, isSelected = false) => {
    ...
    const percentage = resultData.percentage || calculatePercentage(resultData.marks, resultData.class);
    const grade = resultData.grade || getGrade(percentage);

    return (
      <div className={`bg-white rounded-2xl shadow-xl overflow-hidden mb-6 ${isSelected ? 'ring-2 ring-[#B8860B]' : ''}`}>
        {resultData.rank && (
          <div className="bg-gradient-to-r from-[#B8860B] to-[#0B3B2C] px-6 py-4 text-center">
            <div className="text-white">
              <div className="text-3xl font-bold">{getRankBadge(resultData.rank)}</div>
        ...
        <div className="p-6">
          <div className="text-center mb-6">
            <div className="inline-block bg-[#0B3B2C]/10 rounded-full px-4 py-1">
              <span className="text-sm font-semibold text-[#B8860B]">
                {resultData.class} Class - TALENT EXAM RESULT {new Date().getFullYear()}
              </span>
            </div>
          </div>
          ...
          <div className="space-y-4">
            ...
            <div className="flex flex-wrap justify-between py-3 border-b border-gray-200">
              ...
              <span className="text-2xl font-bold text-[#B8860B]">{resultData.marks} / {totalMarks}</span>
            </div>
            <div className="flex flex-wrap justify-between py-3 border-b border-gray-200">
            ...
            <div className="flex flex-wrap justify-between py-3 border-b border-gray-200">
              ...
              <span className={`px-3 py-1 rounded-full text-sm font-semibold ${
                ...
                {resultData.resultStatus === "pass" ? "✓ PASS" : "✗ FAIL"}
              </span>
            </div>
          </div>
        ...
        <div className="bg-white rounded-2xl shadow-xl overflow-hidden">
          ...
          <div className="p-6">
            ...
            <div className="mt-4 text-sm text-gray-500">
              ...
              <p className="mt-1 text-xs text-[#B8860B]">📌 Note: 3rd & 4th class: 50 marks total | 5th-8th class: 60 marks total</p>
            </div>
          </div>
        </div>
                  ...
                  <div className="flex justify-between items-start">
                    ...
                    <div className="text-right">
                      <span className={`px-2 py-1 rounded-full text-xs font-semibold ${
                        res.resultStatus === "pass" ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"
                      }`}>
                        {res.resultStatus === "pass" ? "PASS" : "FAIL"}
                      </span>
                      <p className="text-lg font-bold text-[#B8860B] mt-1">{res.marks} marks</p>
                    </div>
                  </div>
                  <div className="mt-3 pt-3 border-t border-gray-100 text-center">
                    <span className="text-xs text-[#B8860B]">Click to {selectedResult?._id === res._id ? "hide" : "view"} full result</span>
                  </div>
                </div>
              ))}
            </div>
